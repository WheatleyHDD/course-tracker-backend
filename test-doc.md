...
### Получить заявки всех пользователей за определенный период

`POST /api/applications/date`
Обязательные параметры:
- `date_from` - первый день месяца
- `date_to` - последний день месяца
Необязательные параметры:
- `limit` - Количество записей от 10 до 100 (по умолчанию 10)
- `page` - Страница записей
...
- `start_date` - начало обучения
- `end_date` - конец обучения
- `count` - количество заявок и курсов

### Получить заявки всех пользователей по названию и периоду (к предыдущему методу)

`POST /api/applications/getByGroup`
Обязательные параметры:
- `course_name` - Название курса
- `start_date` - Начало обучения
- `end_date` - Конец обучения
...
`POST /api/courses/names`

Лимит в 10 записей

Обязательный параметр:
- `like` - Начало названия для упрощения поиска

Ответ в `response`:

- Массив со строками отсортированный по возрастанию